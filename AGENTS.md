...
- **列过滤 + 冻结行列 + 数字格式化(视图层)**:列过滤(文本/数值/值集/空白,多列 AND)重扫描在 `crates/core/src/filter.rs`,`WasmSheet` 持「可视行→底层行」映射(可视行连续 `0..V`,渲染几何复用);**冻结行列**在 `GridLayout` 记录冻结跨度,冻结时走四象限全量重绘隔离路径;**数字格式化** `core/numfmt.rs`。见 [RFC-0005](./docs/rfcs/0005-view-filter-freeze.md) / [RFC-0006](./docs/rfcs/0006-word-excel-ppt-readonly.md)。
...
- **文档页(Word 只读)**:`core/docx.rs` 用 docx-rs 读路径抽出平面化模型(段落/标题/对齐/**有序无序列表(查 numbering.xml)**/图片/表格),`web/word` 流式布局 + sticky canvas 纵向虚拟化渲染(文字/加粗斜体/颜色/对齐含**两端对齐**/列表/图文混排/表格)。见 [RFC-0006](./docs/rfcs/0006-word-excel-ppt-readonly.md)。
- **演示页(PPT 只读)**:`core/pptx.rs` 用 zip+quick-xml 直接解析 PresentationML → 幻灯模型(文本框/图片/自选图形/对齐,EMU÷9525,**占位符继承版式/母版几何**、**主题 schemeClr**),`web/ppt` canvas 渲染 + 缩略图导航 + **全屏演示模式**。见 [RFC-0006](./docs/rfcs/0006-word-excel-ppt-readonly.md)。
