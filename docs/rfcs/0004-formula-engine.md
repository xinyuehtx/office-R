...
- **依赖图 / 依赖路径分析**(`graph.rs`):从公式 AST 提取**前驱**(它读取的单元格 + 范围),
  ...
  范围读者还建了**列索引**(列 → 覆盖该列的窄范围公式,列跨度 > 64 的宽范围归入回退集合),
  编辑某格时只查它所在列的候选,而非扫描全部范围公式。
...
- **循环更新策略**:拓扑排序识别出环(及其下游)。
  ...
  - **迭代计算**(`set_iterative(true, max_iter, epsilon)`,对应 Excel「启用迭代计算」):
    对环迭代求值,数值最大变化 < `epsilon` 或达到 `max_iter` 即停。估计值放进求值器缓存,
    故环内互引直接命中缓存、不再递归。迭代法可选 **Jacobi**(默认,整轮同步更新)或
    **Gauss–Seidel**(`set_iterative_method`,就地更新,通常收敛更快)。
...
- 动态数组溢出与 `@` 隐式交集。
- 范围依赖已用**列索引 + 宽范围回退**加速;若列跨度极宽的范围公式很多,可再引入二维区间树
  (如 HyperFormula)进一步降复杂度。
- 迭代计算已支持 **Jacobi / Gauss–Seidel** 两种方法;更复杂的收敛加速(如 SOR 超松弛)暂未做。
